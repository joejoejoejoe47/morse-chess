...
async function prune(sql: Sql) {
  await Promise.all([
    sql.query(`DELETE FROM webrtc_signals WHERE created_at < now() - interval '60 seconds'`),
    sql.query(`DELETE FROM webrtc_peers WHERE last_seen < now() - interval '30 seconds'`),
  ]);
}
