...
import { hasClubSession, markClubSession, consumeOauthPending, subscribeClubSession } from "./club-session";
...
export function useClubDoor() {
  ...
  useEffect(() => {
    if (user && !club && consumeOauthPending()) markClubSession();
  }, [user, club]);

  if (isPending) return { status: "pending" as const, user };
  if (!club || !user) return { status: "auth" as const, user: null };
